...
| max（当前默认） | 2048 / depth 720 / 100 steps | 约22分钟；24个候选经后处理只剩12层，质量分59.15 | 运行成功但业务质量不通过 |

`max` 运行时 GPU 利用率 100%，显存约 22 GiB，功耗约 343 W。瓶颈是计算量而不是显存。1024 档比 512 档层数更少也再次证明：提高分辨率和步数不会单调改善语义拆分完整度。

本次 2048 完整产物的逐项证据、自动质量门和优化复跑命令见
[H20 最高参数低质量复盘](SEETHROUGH_H20_LOW_QUALITY_ANALYSIS.zh-CN.md)。

早期临时实例曾在最高配任务结束前停止接受 SSH，导致当轮产物没有及时拉回；后续实例已按“每轮完成立即归档”的方式取得本次 2048 完整样本。今后仍应在每个成功档位结束后立即归档和下载，不要等最高配一起打包。
...
1. **分级跑测**：probe 验证链路，screen 做多 Seed，balanced/quality 只跑入选 Seed。不要直接 max。
2. **降低 Steps**：Seed 初筛默认12步，终稿通常30–40步；100步不保证边界或漏件改善。
...
| 快筛 | 768 / 512 / 12 | 4 | 自动排除漏件和明显错分 |
| 复筛 | 1024 / 720 / 30 | 1–2 | 比较语义、Alpha 和重组 |
| 终稿 | 1024 / 720 / 40 | 1–2 | 输出候选部件 |
| 压测 | 2048 / 720 / 100 | 0–1 | 只做RGBA性能/上限验证 |
