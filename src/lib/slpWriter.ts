...
async function getH5Module(): Promise<H5Module> {
  if (!modulePromise) {
    modulePromise = (async () => {
      // Always use browser ESM build — Vite aliases h5wasm/node to the same bundle
...
function getH5FileSystem(module: H5Module): H5FileSystem {
...
/**
 * Serialize a Labels object to SLP bytes (HDF5 format).
 *
 * This creates an in-memory HDF5 file using h5wasm's MEMFS virtual filesystem,
 * writes all label data into it, then reads back the bytes.
 */
export async function writeSlpToBytes(labels: Labels): Promise<Uint8Array> {
  const module = await getH5Module();
  const fs = getH5FileSystem(module);
  const virtualPath = `/slp-write-${Date.now()}-${Math.random().toString(16).slice(2)}.slp`;

  const file = new module.File(virtualPath, "w");
  try {
  ...
  try {
    const bytes = fs.readFile!(virtualPath);
    return bytes;
  } finally {
...
function serializeSkeletons(
  skeletons: Skeleton[]
): { skeletons: any[]; nodes: Array<{ name: string }> } {
...
function writeVideos(file: any, videos: Video[]): void {
  const payload = videos.map((video) =>
    JSON.stringify(serializeVideo(video))
  );
...
function serializeVideo(video: Video): Record<string, unknown> {
  const backend = {
    ...(video.backendMetadata ?? {}),
  } as Record<string, unknown>;
  if (backend.filename == null) backend.filename = video.filename;
  if (backend.dataset == null && video.backend?.dataset)
    backend.dataset = video.backend.dataset;
  if (backend.shape == null && video.backend?.shape)
    backend.shape = video.backend.shape;
  if (backend.fps == null && video.backend?.fps != null)
    backend.fps = video.backend.fps;
...
function writeSessions(
  ...
  const payload = sessions.map((session) =>
    JSON.stringify(serializeSession(session, videos, labeledFrameIndex))
  );
...
function serializeSession(
  ...
  for (const frameGroup of session.frameGroups.values()) {
    ...
    frame_group_dicts.push(
      serializeFrameGroup(frameGroup, session, labeledFrameIndex)
    );
  }
...
function serializeFrameGroup(
  ...
  const instance_groups = frameGroup.instanceGroups.map((group) =>
    serializeInstanceGroup(group, session)
  );
...
function writeLabeledFrames(file: any, labels: Labels): void {
  ...
  for (const labeledFrame of labels.labeledFrames) {
    ...
    const videoIndex = Math.max(
      0,
      labels.videos.indexOf(labeledFrame.video)
    );
